...
(reg-event-db :news-resp
			  (fn [db [_ res]]
				  (update-in db [:root-db] assoc :loading-news? true :data-news (keywordize-keys (js->clj res)))))

(reg-event-db :news-error
			  (fn [db [_ res]]
				  (update-in db [:root-db] assoc :loading-news? true :error-news (keywordize-keys (js->clj res)))))
